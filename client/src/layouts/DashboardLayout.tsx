...
export const DashboardLayout: React.FC = () => {
  ...
  const sidebarContent = (onNavigate?: () => void) => (
    <>
      <div className="flex flex-col flex-grow overflow-y-auto scrollbar-none mb-6">
        {/* Logo */}
        <Link to="/" onClick={onNavigate} className="flex items-center gap-2 mb-8 shrink-0">
        ...
        <nav className="space-y-1">
          <Link to="/" onClick={onNavigate} className={`${linkClass('/')} flex items-center gap-2.5`}>
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
            </svg>
            <span>Home</span>
          </Link>
          <Link to="/dashboard" onClick={onNavigate} className={`${linkClass('/dashboard')} flex items-center gap-2.5`}>
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
          ...
          <Link to="/dashboard/projects" onClick={onNavigate} className={`${linkClass('/dashboard/projects')} flex items-center gap-2.5`}>
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
          ...
          <Link to="/dashboard/collaborations" onClick={onNavigate} className={`${linkClass('/dashboard/collaborations')} flex items-center gap-2.5`}>
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" />
            </svg>
            <span>Collaborations</span>
          </Link>
          <Link to="/dashboard/applications" onClick={onNavigate} className={`${linkClass('/dashboard/applications')} flex items-center gap-2.5`}>
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
            </svg>
            <span>Applications</span>
          </Link>
          <Link to="/dashboard/events" onClick={onNavigate} className={`${linkClass('/dashboard/events')} flex items-center gap-2.5`}>
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
            </svg>
            <span>Events</span>
          </Link>
          <Link to="/dashboard/activity" onClick={onNavigate} className={`${linkClass('/dashboard/activity')} flex items-center gap-2.5`}>
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
            </svg>
            <span>Activity</span>
          </Link>
          <Link to="/dashboard/settings" onClick={onNavigate} className={`${linkClass('/dashboard/settings')} flex items-center gap-2.5`}>
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
          ...
          {user?.role === 'admin' && (
            <div className="pt-4 mt-4 border-t border-slate-100 space-y-1">
              ...
              <Link to="/dashboard/admin" onClick={onNavigate} className={`${linkClass('/dashboard/admin')} flex items-center gap-2.5`}>
                <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
              ...
              <Link to="/dashboard/admin/users" onClick={onNavigate} className={`${linkClass('/dashboard/admin/users')} flex items-center gap-2.5`}>
                <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
              ...
              <Link to="/dashboard/admin/users?status=pending" onClick={onNavigate} className={`${linkClass('/dashboard/admin/users?status=pending')} flex items-center gap-2.5`}>
              ...
              <Link to="/dashboard/admin/moderation" onClick={onNavigate} className={`${linkClass('/dashboard/admin/moderation')} flex items-center gap-2.5`}>
                <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
                <span>Mod Queue</span>
              </Link>
              <Link to="/dashboard/admin/analytics" onClick={onNavigate} className={`${linkClass('/dashboard/admin/analytics')} flex items-center gap-2.5`}>
                <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
  ...
  return (
    <div className="flex min-h-screen bg-slate-50/50">
      {/* Sidebar (Left) - hidden below md, shown on tablet+ */}
      <aside className="hidden md:flex w-64 border-r border-slate-100 bg-white p-6 flex-col justify-between shrink-0 sticky top-0 h-screen select-none z-50">
        {sidebarContent()}
      </aside>
      ...
      <div className="flex-grow flex flex-col min-h-screen min-w-0">
        {/* Header bar */}
        <header className="h-16 bg-white border-b border-slate-100 px-4 md:px-8 flex items-center justify-between sticky top-0 z-40 select-none shrink-0 gap-3">
          <div className="flex items-center gap-2 min-w-0">
            {/* Mobile menu toggle */}
            <button
              onClick={() => setMobileSidebarOpen(true)}
              aria-label="Open menu"
              className="md:hidden p-2 -ml-2 text-[#5c7075] hover:text-[#091e22] rounded-xl hover:bg-slate-50 cursor-pointer shrink-0"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
              </svg>
            </button>
            <div className="text-xs text-[#5c7075] font-semibold flex items-center gap-2 min-w-0">
              <span className="hidden sm:inline shrink-0">Organization</span>
              <span className="hidden sm:inline text-slate-350 shrink-0">&gt;</span>
              <span className="text-[#091e22] font-bold truncate">{getBreadcrumb()}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            <Link
              ...
              className="text-slate-600 hover:text-[#006655] cursor-pointer relative block p-1 hover:scale-105 transition-transform"
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute -top-1.5 -right-2 bg-red-500 text-white text-[9px] font-black rounded-full h-4 min-w-[16px] px-1 flex items-center justify-center border-2 border-white shadow-sm">
            ...
            {user?.role === 'admin' && (
              <Link
                to="/dashboard/admin"
                className="hidden md:inline-flex bg-emerald-50 hover:bg-emerald-100 text-[#006655] border border-emerald-200 px-3 py-1 rounded-lg text-xs font-bold transition-all shadow-xs shrink-0 items-center gap-1.5"
            ...
            <Link 
              ...
              className="relative w-8 h-8 rounded-full overflow-hidden bg-[#e6f7f8] border-2 border-[#006655]/10 hover:border-[#006655] shadow-sm select-none cursor-pointer flex items-center justify-center transition-all hover:scale-105 shrink-0"
      ...
      {mobileSidebarOpen && (
        <div className="fixed inset-0 z-[90] md:hidden">
          <div
            ...
            aria-hidden="true"
          />
          <div className="absolute inset-y-0 left-0 w-72 max-w-[85%] bg-white dark:bg-[#121e21] border-r border-slate-100 p-6 flex flex-col shadow-2xl overflow-y-auto scrollbar-none">
            <div className="flex justify-end mb-2 shrink-0">
              <button
                onClick={() => setMobileSidebarOpen(false)}
                aria-label="Close menu"
                className="p-2 text-[#5c7075] hover:text-[#091e22] rounded-xl hover:bg-slate-50 cursor-pointer"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
      ...
      {showConfirmLogout && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] w-[calc(100vw-2rem)] max-w-md flex flex-col sm:flex-row items-center gap-4 bg-white border border-slate-100 text-[#091e22] px-6 py-4 rounded-2xl shadow-2xl text-xs font-bold animate-slide-in select-none dark:bg-[#121e21] dark:border-[#1e2e30] dark:text-[#f1f5f9]">
          <div className="flex items-center gap-2 min-w-0">
